...
@ti.func
def get_atmos_density(pos: vec3):
...
def sample_interaction_delta_tracking(ray_pos: vec3, 
    ...
    while t < t_max:
        ...
        extinction_sample = extinctions * get_atmos_density(ray_pos)
...
def sample_interaction(ray_pos: vec3, 
    ...
    rmo_event, rmo_t, rmo_id = sample_interaction_delta_tracking(ray_pos, ray_dir, t_start, t_max, rmo_extinctions, max_extinction_rmo)
    ...
    if rmo_event == NULL_EVENT or rmo_t > t_start:
        ...
        cloud_event, cloud_t, _ = sample_interaction_delta_tracking(ray_pos, ray_dir, t_start, t_max, cloud_extinctions, max_extinction_cloud)
...
def sample_transmittance(ray_pos: vec3, 
    ...
    if atmos_isection.y < 0.0: 
        t_max = -1.0 # ray doesnt cross atmosphere
    rmo_extinctions = vec4(extinctions.xyz, 0.0)
    transmittance  = transmittance_ratio_tracking(ray_pos, ray_dir, t_start, t_max, rmo_extinctions, max_extinction_rmo)
    
    t_start, t_max = intersect_cloud_limits(ray_pos, ray_dir, land_isection)
    cloud_extinctions = vec4(0.0, 0.0, 0.0, extinctions.w)
    transmittance *= transmittance_ratio_tracking(ray_pos, ray_dir, t_start, t_max, cloud_extinctions, max_extinction_cloud)
    return transmittance
...
def evaluate_phase(ray_dir: vec3, light_dir: vec3, interaction_id: ti.i32, reduce_peak):
    ...
    if interaction_id == volume.RAYLEIGH_ID:
        ...
        phase += volume.mie_phase(cos_theta, reduce_peak)
...
def sample_phase(ray_dir: vec3, interaction_id: ti.i32, reduce_peak):
    ...
    if interaction_id == volume.RAYLEIGH_ID or interaction_id == volume.ISOTROPIC_CLOUD_ID:
        ...
        sample_dir = volume.sample_mie_phase(ray_dir, reduce_peak)
    else:
...
def path_tracer(path: PathParameters,
    ...
    in_scattering = 0.0
    throughput = 1.0
    for scatter_count in range(0, 10):
        ...
        earth_intersection = rsi(ray_pos, ray_dir, volume.planet_r).x
        ...
        if event == ABSORB_EVENT:
            ...
            land_normal = sphere_normal # land_normal(height_sampler, land_pos, scene.land_height_scale)
            # albedo_srgb, ocean, bathymetry, emissive_factor = get_land_material(albedo_sampler, 
            #                                                                     ocean_sampler, 
            #                                                                     bathymetry_sampler, 
            #                                                                     emissive_sampler,
            #                                                                     land_pos)
            albedo_srgb = vec3(0.3)
            ocean = 0.0
            bathymetry = 0.0
            emissive_factor = 0.0
            ...
            direct_visibility = rsi(offset_pos, light_dir, volume.planet_r).x < 0.0# intersect_land(height_sampler, offset_pos, light_dir, scene.land_height_scale) < 0.0
